...
def batched_learn_params_ridge_regression(
        ...
        holdout_size, add_bias=True, mst_nonlinearity=None, mst_zcore=True, shuffle=False, view_angle=1., dtype=np.float32):
    ...
    for mr, ml in iterate_range(0,len(models),model_batch_size):
        ...
        mst_data, mst_avg, mst_std = model_space_tensor(datas, \
            ...
            zscore=mst_zcore, trn_size=trn_size,\
        ...
        if np.sum(imp)>0:
            ...
            best_lambdas[arv] = selected_lambdas[imp]
            best_scores[arv] = selected_scores[imp]
            for k in range(len(best_w_params)):
                best_w_params[k][arv] = selected_w_params[k][imp]
            best_rf_params[arv] = imp_rf_params
            best_avg[arv] = imp_avg
            best_std[arv] = imp_std
