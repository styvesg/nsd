...
def learn_params_ridge_regression(data, voxels, _fmaps_fn, models, lambdas, aperture=1.0, _nonlinearity=None, zscore=False, sample_batch_size=100, voxel_batch_size=100, holdout_size=100, shuffle=True, add_bias=False):
    ...
    _fmaps = _fmaps_fn(data[:sample_batch_size])
    device = _fmaps[0].device
    ...
    with torch.no_grad():
        for m,(x,y,sigma) in enumerate(models):
            ...
            for rt,rl in iterate_range(0, nt, sample_batch_size):
                _mst = torch.cat([torch.tensordot(_fm, _pf, dims=[[2,3], [0,1]]) for _fm,_pf in zip(_fmaps_fn(data[rt]), _pfs)], dim=1) # [#samples, #features]
                if _nonlinearity is not None:
                    _mst = _nonlinearity(_mst)
                mst[rt] = get_value(_mst)

            if zscore:  
                mstm = np.mean(mst[:trn_size], axis=0, keepdims=True)
                msts = np.std(mst[:trn_size], axis=0, keepdims=True) + 1e-6          
                mst -= mstm
                mst /= msts    
    ...
    if zscore:  
        ...
        return best_losses, best_lambdas, models[best_models], return_params, None, None
    ...
    with torch.no_grad():
        for rt,lt in iterate_range(0, nt, sample_batch_size):
            ...
            for rv,lv in iterate_range(0, nv, voxel_batch_size):
                ...
                if mst_avg is not None:
                    _mstm = torch.t(_to_torch(mst_avg[rv], device=device)) # [#features, #voxels]
                if mst_std is not None:
                    _msts = torch.t(_to_torch(mst_std[rv], device=device)) # [#features, #voxels]
                _pars = [_to_torch(p[rv], device=device) for p in params] # [#voxels, #features]...

                _pfs = [_to_torch(pnu.make_gaussian_mass_stack(models[rv,0], models[rv,1], models[rv,2], n_pix, size=aperture, dtype=dtype)[2], device=device) for n_pix in fmaps_rez] # [nv, nx, nx]
